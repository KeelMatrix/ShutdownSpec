...
The first command compares the supported .NET 8 runtime; the second compares the targeted current .NET 10 hosting/runtime surface. A run fails if the complete loop exceeds 120 seconds or if any independent outcome/checkpoint differs. Record the exact runtime, duration, iteration count, and clean/broken outcome counts for Windows and Linux. The public macOS job still runs the full Release test suite and package gate; its runner does not run this several-thousand-iteration fixture.
