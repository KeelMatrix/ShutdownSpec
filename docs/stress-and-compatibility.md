...
The isolated fixtures do not reference ShutdownSpec. The .NET 8 fixture requests `Microsoft.Extensions.Hosting` 8.0.1, but the exact-candidate Windows and Linux runs load `Microsoft.Extensions.Hosting.Abstractions` and report informational version 8.0.10. The .NET 10 fixture requests and reports 10.0.12. The requested package version and the hosting assembly/runtime version actually executed are distinct; neither isolated fixture changes the shipping package dependency boundary. Each fixture proves host startup, background execution entry, stopping-token delivery, host stop completion, and cleanup, and prints the resolved assembly/informational version. The supported compatibility claim is maintained in the [canonical lifecycle contract](contract.md#supported-platforms-and-dependency-boundary).
